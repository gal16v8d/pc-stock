...
import co.com.gsdd.pcstock.model.PCStockFile;
...
public abstract class AbstractDirectoryHelper<T extends PCStockFile> {
    ...
    private List<File> filterDirectories(File dir) {
        return Optional.ofNullable(dir).map(directory -> directory.listFiles((FileFilter) DirectoryFileFilter.INSTANCE))
    ...
    private List<CompareFile> iterateCompareFileList(List<T> cL1, List<T> cL2) {
        ...
        for (T psStockFile : cL2) {
            compareList.add(new CompareFile(psStockFile.getName(), 0L, extractAccountField(psStockFile), true));
        }
        return compareList;
    }
